...
if [ -z "$CLUSTER_JOIN" ] && [ "$1" = 'mysqld' -a -z "$wantHelp" ]; then
	...
	if [ ! -d "$DATADIR/mysql" ]; then
		...
		# save uuid rainbond,zq modify in 2021-03-22
		echo mysql | sudo -S touch /var/lib/mysql/wsrep_local_state_uuid.log
		...
		# use in the check script,zq modify in 2021-04-19
		file_env 'MONITOR_HOST' '%'
