...
function main() {
    ...
    # Gets the dependent component information for rainbond,zq modify in 2021-04-19
    CHECK_AVAILABLE_PXC_NODES=$(/usr/bin/nslookup ${DEPEND_SERVICE%:*} | /usr/bin/grep -o 'Name' | /usr/bin/wc -l)
    for((i=0;i<$CHECK_AVAILABLE_PXC_NODES;i++)); do
        PXC_CLUSTER_HOST+=($(nslookup ${DEPEND_SERVICE%:*} | grep Name | awk '{print $2}' | cut -d . -f -1 | sort --version-sort -r | uniq)-$i.$(nslookup ${DEPEND_SERVICE%:*} | grep Name | awk '{print $2}' | sort --version-sort -r | uniq))
    done

    for pxc_host in ${PXC_CLUSTER_HOST[@]}; do
        if [ -z "$pxc_host" ]; then
