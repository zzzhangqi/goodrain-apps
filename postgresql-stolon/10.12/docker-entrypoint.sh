...
if ! _is_sourced; then
    ...
    keeper)
        ...
        ;;
    sentinel)
        _main gosu stolon stolon-sentinel --cluster-name stolon-cluster \
        --store-backend=consul \
        --store-endpoints http://$Consul0:8500,http://$Consul1:8500,http://$Consul2:8500 \
        --log-level debug
        ;;
    proxy)
        _main gosu stolon stolon-proxy \
        ...
        ;;
    *)
        echo 'You must tell me your role in the cluster, let $STOLON_ROLE=keeper sentinel or proxy'
        exit 1
        ;;
    esac
fi
